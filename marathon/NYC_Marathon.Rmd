---
title: 'NYC Marathon (an2886, ap3635, al3273, ah3617)'
output:
...
* `type`: the inferred group category. "R" is for runners, "H" for hand clyclists and "W" for wheelchair.
...
![View of the webpage that we scrapped](./pics/webpage.png)
...
Web scrapping was more time consuming than we thought, since the website was built under javascript in order to scrape we needed the use of `selenium` which is an is a WebDriver tool for automation that basically connects to a driver and retrieves the rendered html. From the html we were able to obtain the data using a really well-develop package of `BeautifulSoup` but it still required us to overcome two main obstacles (1) understanding the web scrapping process and (2) to come up with the template that our program should use in order to find the information.

To get a complete understanding of the process, we made intensive use of different resources online: either YouTube videos or other Q&A websites such as `StackOverflow`. A particular source that we found useful was this article  [(link)](https://medium.freecodecamp.org/better-web-scraping-in-python-with-selenium-beautiful-soup-and-pandas-d6390592e251). Moreover, we became acquainted with the myriad of details that were not evident when we started the process. First, we had to distribute our work in a computer engine on the cloud. The difficulty 
is that opening a web driver each time to obtain the render html is a really slow process, specially that we needed for each finisher. For example, we have to run a long `for-loop` were we have to make a different connection for each of the over 50 K participants of the marathon for each year (this times the number of year that we downloaded). Additionally, we had to set-up the computer and parallelize the process and let it do the work (which took approximately 3 days)

In term of coming up with a template for our program to run. We had to learn (actually before starting D3) the structure of the html website and embed that knowledge into a script for `BeautifulSoup` to perform its magic. Furthermore, this is a sensitive procedure. Altering any location of an element in the web page renders the script useless. Thus, on the one hand, we had to make several "robustness checks" for our script before letting it run (because we risk losing days of work). The good thing is that the same template was able to be used for all participants across the four years of data we scrapped.
...
Outliers in this context come either from poor performing runners, excellent performing runners and other type of competitions categories (wheelchair and hand cyclists). Thus, the notion of _outlier_ here stems from mixing many different cohorts of people (professional athletes, standard competitors, wheelchair and hand cyclists) in the data rather than by having some plausible error in a certain recording of a variable. Although there were some people that finished the marathon in around 7 hours. But we are uncertain if this is a mistake or maybe they burnt out and finished the race as they could.
...
Performance in athletics is often impacted by the age of the athletes. For example, most of the participants in the Olympics and in international sports competitions tend to be people on the younger end of the spectrum. Since marathon running is a test of endurance, it would be interesting to see if similar trends exist amongst the participants of the NYC Marathon. We can plot density curves of the age of the finishers in each year for which we have data.

From a first glance, we notice that the distribution of the data appears to have an approximately normal shape, with some flattening out at the top of the curve, and a clear mode. The youngest participant is 18, since the rules of the race most likely forbid minors from participating, and the oldest is 87.

Overall, there are more younger people finishing the race in comparison to older people, with a large portion of the participants between the age of 30 and 50, and this trend remains fairly constant across the years. The median age is between 43 in 2015 and 40 in 2018, and there is a slight trend towards the participants becoming younger. This is evidenced by the small leftward shifts in the curves from one year to the next. 
...
### 4.1.2 Age distribution by gender
...
### 4.1.4 Analysis of Teams

All together there are 651 different teams that entered the NYC Marathon Race in the running category. Many of these teams are groups of volunteers that joined their company team or a local charity team. They are also amateur running clubs, and finally professional teams that are sponsored by large athletic equipment firms such as Nike and Adidas. If we look at all of the runners, we see that 39815 of the runners were part of a running team, while 160998 we not, which is over 80% of the runners. Therefore, it's only about 1/5 of the runner that finished the race were part of an organized team. In the graph below, 0 indicates the non-team runners, and 1 indicates the runners on teams. 
...
ggplot(teams, aes(x = team_binary)) + 
  geom_bar(fill="pink") +   
  ggtitle('The majority of people do not belong to a team') +
  theme_bw()
```
...
ggplot(runners, aes(x = team_binary, y = official_time)) +
  ...
  theme_bw() +
  ylab("Official Time") +
  ggtitle('Belonging to a team does not predict better outcomes')
```
...
We can group the location data into three groups: local, national, and international. From the table below, we see that the proportion of representation that these three groups receives remains fairly constant over the 4 years. Around 40% of the finishers are local, meaning that they reside in New York. There are around 37-40% representing the USA excluding those from NY. And finally, around 20% of the participants come to NY from other countries in the world. This information is summarized in the bar chart below where the data has been aggregated across years. 
...
Excluding New Yorkers from the data, we can further investigate which US States are most represented amongst the marathon finishers. In the horizontal bar chart we see that after New York, New Jersey has the most participants, followed by California and Massachusettes. These results are not surprising as New Jersey and Massachusettes are very close to New York. Additionally, California is the most populous state therefore is expected that it would be heavily represented, despite its lack of proximity to New York. 
...
We can examine a similar plot at the country level. After the US, Italy by far has the most represented country, followed by several other Western European countries, China, Brazil, Mexico, Canada and Australia. Is is interesting to note that Europeans have higher rates of participation in the marathon compared to countries that are close to the US, including immediate neighbors Mexico and Canada. 
...
g2 <- ggplot(a2, aes(x=country, y=count)) + 
  ...
  theme_minimal(24) + 
    ...
    panel.grid.major.x=element_blank(),
    panel.background = element_blank(), #element_rect(fill = "white", colour = "lightgray"),
    panel.grid.major.y = element_blank(), #= element_line(linetype=3,
  axis.title.y = element_blank(),
  axis.title.x = element_blank()
  )
g2
...
The ratio of men and women participants in sports has long been an important topic of discussion. We can examine to what extent women are equally represented amongst the NYC marathon finishers according to their country of origin. In the cleveland dot plot below we plot the fraction of runners from a country that are women, with the country names on the y-axis ordered by decreasing proportion. The vertical line indicates equal proportion. 

We note that in 2018, only New Zealand is represented more highly by women than by men. Russia is 19th on the list having a proportion of around 0.5, meaning that the remaining 122 countries represented had more than double the number of men compared to the number of women. From the table we see that the aggregate proportion has remained relatively stable over the last 4 years at around 0.7 for the runners. But from the visualization it is clear that many of these women are coming from only a handful of countries. 
...
On the US state level, however, women are the majority of finishers for 27 states, including California which is the third most represented state. Many of the other states in the aforementioned 27, however, do not have many participants, and so the ratio of women to men is close to, but not equal to 1. 
...
Visualizing the change in these ratios over time shows us that many states and countries have had a positive change in the number of women they have participating in the NYC marathon from 2015 to 2018. We can note the trend that many Southern and Mid-Western states including Louisiana, Alabama, Kentucky, Tennessee, and Kansas have increased their fraction. Similarly, several countries have increased the number the fraction of women representing their country several times over. 

Despite all these positive individual country-level and state-level shifts in the gender break-down of the people representing their countries or states, the overall ratio of women to men has remained roughly constant over the past four years. Therefore, while some locations are sending more women, others are sending less, and the fraction remains constant. 
...
1. In terms of location, __we find that athletes mostly come from the US (over 70%) and the rest are international__. In terms of age and gender, __we found that the majority of the participants are young adults around 40 years old and that, on average, female athletes tend to be younger__. Also, we say that as age increases the number of female participants decreases. 

\vspace{6pt}

```{r Two Pannel Plot, fig.height=3, echo=FALSE, warning=FALSE, message=FALSE}
g0 <- ggplot(a0, aes(x=location_type)) + 
  geom_bar(aes(y = (..count..)/sum(..count..))) +
  scale_y_continuous(labels = function(x) paste0(x*100, "%")) + 
  ggtitle('Location of Runners') +
  xlab('') +
    theme_bw() + 
    theme(
    panel.grid.major.x=element_blank(),
...
2. We found that the gender ratio is generally low and that only 3 countries are above 0.8 (women / men). Nonetheless, in the majority of the US states have an above 1 ratio. Please see section 4.1 for the full decomposition of each graph.

\vspace{6pt}

```{r Gender ratio, fig.height=3, echo=FALSE, warning=FALSE, message=FALSE}
df3 <- marathon %>% 
  filter(type == 'R') %>%
  filter(state_name != 'NA') %>% 
  group_by(state_name, year) %>% 
  ...
  arrange(desc(ratio_gender))
df3$over_60 = '< 1'
df3$over_60[df3$ratio_gender > 1] = '[1, 1.25]'
df3$over_60[df3$ratio_gender > 1.25] = '> 1.25'
df3$over_60 <- factor(df3$over_60, levels = c('< 1', '[1, 1.25]', '> 1.25'))
df3 <- df3 %>% group_by(over_60) %>% 
  summarize(count = n())

gender3 <- ggplot(data = df3, aes(x = over_60, y = count)) +
  geom_col() +
  labs(title = 'Almost states all above 1',
       subtitle = 'Women / men for US states 2018') +
  ylab('States') +
  xlab('') +
  theme_bw() + 
...
df4$over_60 = '< 0.5'
df4$over_60[df4$ratio_gender > 0.5] = '[0.5, 0.75]'
df4$over_60[df4$ratio_gender > 0.75] = '> 0.75'
df4$over_60 <- factor(df4$over_60, levels = c('< 0.5', '[0.5, 0.75]', '> 0.75'))
df4 <- df4 %>% group_by(over_60) %>% 
  summarize(count = n())

gender4 <- ggplot(data = df4, aes(x = over_60, y = count)) +
  geom_col() +
  labs(title = 'Few countries above 0.5',
       subtitle = 'Women / men for Countries 2018') +
  ylab('Countries') +
  xlab('') +
  theme_bw() + 
  theme(plot.title = element_text(size = 8),
...
gridExtra::grid.arrange(gender4, gender3, ncol=2)
```

\vspace{6pt}

3. We found that the difference in performance between women and men is only observed in groups that rely on more capacity (for full details see section 4.2).

\vspace{6pt}

```{r echo=FALSE, fig.height=3}
median15 <- r15 %>%  filter(gender != 'NA') %>%
  group_by(wave, gender) %>% 
  summarize(med_time = median(official_time)) %>% 
  spread(key = gender, value = med_time) %>% 
  mutate(diff = as.numeric((F - M)) / 60)

wave1 <- ggplot(data = median15, aes(x = wave, y = diff)) +
  geom_col() +
  ylab('Median Difference (min)') +
  xlab('') +
  theme_bw() + 
  ggtitle('Waves 1 and 4 show differences') +
  theme(plot.title = element_text(size = 8),
        plot.subtitle = element_text(size = 8),
        axis.text.y = element_text(size = 8),
        axis.text = element_text(size = 8),
...
wvsm <- marathon %>% filter(gender != 'NA', year == 2018)

wave2 <- ggplot(data = wvsm, aes(x = factor(gender, levels = unique(gender)) , y = official_time)) +
  geom_boxplot(fill = "royalblue2", outlier.colour = "darkorange1") + 
  ylab("Official Time") +
  xlab("Year") +
  theme_bw() + 
  ggtitle("Overall men perform better than women") +
...
gridExtra::grid.arrange(wave2, wave1, ncol=2)
```

\vspace{6pt}

4. Finally, __we saw that we could summarize the whole race by using an embedding__. This embedding allowed us to visualize (1) how close the race was by looking at the distance between points, (2) what is the variance of the data set by looking at the spread on the y-axis, (3) if odd events happened by noticing the dots that were apart from their closest group. See section 4.3 for all the details

\newpage

# 6. Interactive component
Interactive component can be found in shinnyapps server [(link)](https://anavarrete.shinyapps.io/marathon_dashboard3/])

For the interactive component, we decided to generate a dashboard that could let us summarize the information for each location (states or countries), and compare some results with the overall, across the four years of marathon data we collected. We believe that summarizing data in a graphical way

Creating this App will allow users to choose between countries and states across time, and compare different metrics:

  * The gender-age distribution 
  * The proportion of woman and men and compare it with the rest of the world/states
  * The official time distribution of a specific location compared with the overall
  * The change in terms of performance among athletes, as race advances the distribution widens and for some countries this becomes more clear than for others.

Having summarize information by country and state can be very useful for people interested in marathons, sponsors and even governments to try to promote the participation, specially in countries with not many participants but performing above average (as the case of Kenya). As well, it can encourage the a more equal participation between woman and men by comparing their country/state with the rest of the world/country. We expect that the interactive component of this project can help us understand more about the people participating and the differences in terms of representation and performance compared to the total.

To use this tools one should choose a specific year and location from the sidebar. The location can be to choose between a state from the US or a country. Since there were too many options, we use some shiny Widgets, that do a live-search among the options when typing the first letters.

Some of the key findings in this interactive was looking at the different results, specially for countries like Kenya with a distribution of official times outperforming the general or Taiwan on the opposite side.

\newpage

# 7. Conclusion
Many of the problems that we faced were found in the web scrapping part of the process. As mentioned in previous sections, we had to overcome several challenges in learning how to properly scrape the NY Road Runners website including parsing the webpage and setting up the computer engine. Next came cleaning the data and dealing with messy locations, time format, and making decisions about dealing with missing data. For example, many of the rows needed to be dropped because we could not identify the country in which the city or county resided for that particular runner. This exclusion could have impacted the outcome of our analysis as many city names in the US and Canada are identical to ones found across Europe. Another issue that we had to make a decision about was the handling of data concerning the wheelchair racers and hand-cyclists. There were very few of these participants in comparison to the runners, much to few to dedicate worthwhile insights, and thus those two groups were largely ignored throughout the analysis of performance.

One of the lessons that we learned was the importance of analyzing our data across different variables as opposed to only examining each variable individually. For example, by examining proportion of men and women at the country level, we discovered more information about country-level and state-level trends that we would otherwise explored. More generally, we learned that it is important to be curious and creative with the research directions that we chose in order to extract the most interesting pieces of analysis.

Had there been more time to dig deeper in this analysis, it would have been interesting to add more years in our analysis to analyze trends and shifts in the overall number of people participating, the ratio of women to men, the international representation, and perhaps most interestingly the trends in performance. Has the NYC Marathon become more or less competitive over the years? Another interesting question to look at would be any changes in the geography of the race and how that impacted performance over time? It is a fact that NYC contains a lot of hills that pose challenges to marathon runners, and it is possible that changes in the format of the race could have a significant impact on performance. Finally, a more extensive analysis of the teams participating would give us more insights into how professionals train for a race, and what has been the trend in the impact, if at all, of being a part team on performance. In our analysis we found that teams did not have an effect on the outcome of individual runners, but this could be part of a longer, more dynamic trend. For example, we might discover that many years ago only the professional athletes could train for a marathon successfully, while the amateur solo runners were not able to be competitive.

In summary, we enjoyed completing this analysis and learning more about a famous NYC tradition, and hope to be a part of a data set on marathon finishers one day soon!
